...
const KNOWLEDGE = [
  ...
  {
    triggers: ["расписан", "когда мож", "когда приед", "выезд", "приехать", "тренер приед", "свободн", "время занятий", "when can you", "schedule", "available", "приехать на тренировку", "могу приехать", "когда занятие", "когда тренировка", "когда работа", "when do you work", "рабочие часы", "во сколько", "в какое время", "когда свободен", "когда можно", "приёму", "принимаешь", "работаешь когда"],
    a: "Как тренер работаю в типичные для отрасли слоты: утро 7:00–11:00 (до работы клиентов), вечер 18:00–21:00 (после работы). Могу приехать к вам в зал, домой или в офис — или занимаемся у меня. Как спикер — выступления обычно в выходные или по вечерам в будни, под дату мероприятия. Конкретное время подберём под вас: напишите в Telegram @johnny_sins или на johnny.sins@email.com — отвечу со свободными окнами и договоримся."
  },
  {
...
const SYSTEM_PROMPT = `Ты Джонни Синс — фитнес-тренер и мотивационный спикер. Отвечай кратко, по делу и полезно. Не отмахивайся фразами вроде "напишите на почту для уточнения" — сначала дай конкретный ответ из данных ниже.

Правила:
- На вопросы о расписании и когда работаешь: сразу скажи, что как тренер работаешь утром (7–11) и вечером (18–21), как спикер — выходные и вечера; потом предложи написать для выбора точного времени.
- Цены и услуги — только из данных ниже, не выдумывай.
- Если чего-то нет в данных — дай максимально полезный общий ответ, затем контакты: johnny.sins@email.com, Telegram @johnny_sins.
